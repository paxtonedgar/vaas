...
# =============================================================================
# PHASE 2a: PARAGRAPH NODES
# =============================================================================
# Emit nodes for each content element (paragraph-level granularity).
# This enables:
#   - Proper provenance (typed edges → specific element)
#   - Clean traversal for retrieval + pair-mining
#   - Elimination of orphan concepts (attach to supporting paragraphs)

print("\n--- Emitting Paragraph Nodes ---")

# Roles to skip entirely (artifacts)
SKIP_ROLES_PARA = {ROLE_PAGE_ARTIFACT}

# Header roles - don't create paragraph nodes (they're already anchor nodes)
HEADER_ROLES = {ROLE_BOX_HEADER, ROLE_SECTION_HEADER, ROLE_SUBSECTION_HEADER}

# Build anchor_id -> (anchor_type, box_key) lookup from sections_df
anchor_meta = {}
for _, sec in sections_df.iterrows():
    aid = sec.get("anchor_id")
    if aid:
        anchor_meta[aid] = {
            "anchor_type": sec.get("anchor_type", ""),
            "box_key": sec.get("box_key", ""),
        }

paragraph_nodes = []
for _, elem in elements_df.iterrows():
    # Skip artifacts
    if elem["role"] in SKIP_ROLES_PARA:
        continue

    # Skip header elements - they're already represented as anchor nodes
    if elem["role"] in HEADER_ROLES:
        continue
    ...
    if elem["role"] == ROLE_LIST_BLOCK:
        para_kind = "list"
    else:
        para_kind = "body"

    # Get box_key and anchor_type from anchor metadata
    meta = anchor_meta.get(aid, {})
    box_key = meta.get("box_key", "")
    anchor_type = meta.get("anchor_type", "")

    # Build bbox from geom fields
    bbox = []
    ...
    ro = elem.get("reading_order", 0) if "reading_order" in elements_df.columns else 0
    ro = 0 if pd.isna(ro) else int(ro)

    # Safe page extraction
    page = elem.get("page", 0)
    page = 0 if pd.isna(page) else int(page)

    paragraph_nodes.append({
        "node_id": f"{DOC_ID}:el_{element_id}",
        "doc_id": DOC_ID,
        "node_type": "paragraph",  # Always "paragraph" for element nodes
        "paragraph_kind": para_kind,  # "body" | "list"
        "anchor_id": aid,
        "anchor_type": anchor_type,  # From parent anchor
        "element_id": element_id,
        "box_key": box_key,  # From parent anchor
...
paragraph_nodes_df = pd.DataFrame(paragraph_nodes)
print(f"  Paragraph nodes created: {len(paragraph_nodes_df)}")
if not paragraph_nodes_df.empty:
    print(f"  By kind: {paragraph_nodes_df['paragraph_kind'].value_counts().to_dict()}")

# Append paragraph nodes to graph_nodes
if not paragraph_nodes_df.empty:
    graph_nodes = pd.concat([graph_nodes, paragraph_nodes_df], ignore_index=True)

# =============================================================================
# STRUCTURAL SKELETON: anchor-to-anchor hierarchy + follows edges
# =============================================================================
# This creates the navigable hierarchy that collapses 17 components → 1.
#
# Algorithm (reading order containment):
#   1. Sort all anchors by (page, y0) to get reading order
#   2. Process in order, tracking "current" pointers:
#      - current_section: nearest preceding section
#      - current_box: nearest preceding box (resets on new section)
#   3. Containment rules:
#      - preamble → doc_root
#      - section → doc_root (resets current_box)
#      - box → current_section (or doc_root)
#      - concept → current_box (if exists) OR current_section OR doc_root
#   4. follows edges between consecutive anchors in reading order
...
# Add doc_root node if not present
doc_root_id = f"{DOC_ID}:doc_root"
if doc_root_id not in graph_nodes["node_id"].values:
    graph_nodes = pd.concat([graph_nodes, pd.DataFrame([{
        "node_id": doc_root_id,
        "doc_id": DOC_ID,
        "node_type": "doc_root",
        "anchor_id": "doc_root",
        "box_key": "",
        "label": "1099-DIV Filer Instructions",
        "text": "",
        "pages": [],
        "element_count": 0,
        "char_count": 0,
...
# Sort sections by reading order (page, then y-position from bbox)
def get_sort_key(row):
    pages = row.get("pages", [])
    # Handle both list and scalar pages
...
ordered_sections = sections_df.copy()
ordered_sections["_sort_key"] = ordered_sections.apply(get_sort_key, axis=1)
ordered_sections = ordered_sections.sort_values("_sort_key").reset_index(drop=True)

# Track hierarchy state - "nearest preceding" approach
current_section = None  # Most recent section anchor
current_box = None      # Most recent box anchor (resets on new section)
prev_anchor_id = None   # For follows edges

skeleton_counts = {"parent_of": 0, "in_section": 0, "follows": 0}

for idx, row in ordered_sections.iterrows():
    anchor_id = row["anchor_id"]
    anchor_type = row["anchor_type"]
    node_id = f"{DOC_ID}:{anchor_id}"

    # Extract page for follows edge evidence
    pages = row.get("pages", [])
    if isinstance(pages, (list, tuple, np.ndarray)) and len(pages) > 0:
        page = int(pages[0])
    elif isinstance(pages, (int, float, np.integer)):
    ...
    if anchor_type == "preamble":
        ...
        parent_id = doc_root_id
        edge_type = "parent_of"
        # Update state: this becomes the new current section
        current_section = anchor_id
        current_box = None  # Reset box when entering new section

    elif anchor_type == "box":
        # Box → current_section (or doc_root if no section yet)
        if current_section:
            parent_id = f"{DOC_ID}:{current_section}"
            edge_type = "parent_of"  # Changed from in_section for cleaner hierarchy
        else:
            parent_id = doc_root_id
            edge_type = "parent_of"
        # Update state: this becomes the new current box
        current_box = anchor_id

    elif anchor_type == "subsection":  # concept
        # Concept → current_box (if exists) OR current_section OR doc_root
        # This is the key change: use "nearest preceding box" not "enclosing bbox"
        if current_box:
    ...
    if parent_id and parent_id != node_id:
        edges.append({
            ...
            "source_element_id": None,  # Structural edges derived from layout, not specific element
            "created_by": "structural",
        })
        edge_id += 1
        skeleton_counts[edge_type] = skeleton_counts.get(edge_type, 0) + 1

    # Add follows edge (reading order)
    if prev_anchor_id and prev_anchor_id != anchor_id:
        edges.append({
            ...
            "source_element_id": None,  # Structural edges derived from layout
            "created_by": "structural",
        })
        edge_id += 1
        skeleton_counts["follows"] += 1

    prev_anchor_id = anchor_id

print(f"  parent_of: {skeleton_counts.get('parent_of', 0)}")
print(f"  in_section: {skeleton_counts.get('in_section', 0)}")
print(f"  follows: {skeleton_counts.get('follows', 0)}")

# =============================================================================
# ANCHOR → PARAGRAPH EDGES (Phase 2a)
# =============================================================================
# Connect each paragraph node to its parent anchor node.
# This creates the fine-grained traversal structure for retrieval.

print("\n--- Anchor → Paragraph Edges ---")
anchor_para_count = 0
...
if not paragraph_nodes_df.empty:
    for _, para in paragraph_nodes_df.iterrows():
        ...
        # Verify anchor exists in graph
        if anchor_node_id not in graph_nodes["node_id"].values:
            continue
        ...
        edges.append({
            ...
            "source_element_id": para["element_id"],  # Self-referential for paragraph edges
            "created_by": "structural",
        })
        edge_id += 1
        anchor_para_count += 1

    print(f"  anchor → paragraph edges: {anchor_para_count}")

# =============================================================================
# PARAGRAPH FOLLOWS EDGES (reading order within anchor)
# =============================================================================
# Within each anchor, sort paragraphs by reading order and emit follows edges.

print("\n--- Paragraph Follows Edges ---")
para_follows_count = 0

if not paragraph_nodes_df.empty:
    # Group paragraphs by anchor_id and sort by reading order
    for anchor_id, group in paragraph_nodes_df.groupby("anchor_id"):
        # Sort by reading_order (pages is a list, can't sort directly)
        # reading_order already captures document order
        sorted_paras = group.sort_values("reading_order").reset_index(drop=True)

        # Emit follows edges between consecutive paragraphs
        for i in range(len(sorted_paras) - 1):
            ...
            edges.append({
                ...
                "source_element_id": p1["element_id"],
                "created_by": "structural",
            })
            edge_id += 1
            para_follows_count += 1

    print(f"  paragraph follows edges: {para_follows_count}")

# =============================================================================
# IN_SECTION EDGES (explicit section containment for fast queries)
# =============================================================================
# Denormalized edges: box/concept → section for direct lookups.
# Could be derived from parent_of chain, but explicit edges speed up queries
# like "give me all boxes in Specific Instructions".

print("\n--- In-Section Containment Edges ---")
in_section_count = 0

# Build section_id lookup: track which section each anchor falls under
# (already computed during skeleton building as current_section)
# Re-compute here for explicit edges

anchor_to_section = {}
current_sec = None

for _, row in ordered_sections.iterrows():
    anchor_id = row["anchor_id"]
    anchor_type = row["anchor_type"]

    if anchor_type == "section":
        current_sec = anchor_id
    elif anchor_type in ("box", "subsection") and current_sec:
        anchor_to_section[anchor_id] = current_sec

# Emit in_section edges
for anchor_id, section_id in anchor_to_section.items():
    anchor_node_id = f"{DOC_ID}:{anchor_id}"
    section_node_id = f"{DOC_ID}:{section_id}"

    # Skip if we already have an in_section edge from skeleton building
    # (boxes already get in_section to their parent section)
    # Only add for concepts that might have gone to a box instead
    meta = anchor_meta.get(anchor_id, {})
    if meta.get("anchor_type") == "subsection":
        edges.append({
            "edge_id": f"e_{edge_id}",
            "source_node_id": anchor_node_id,
...
print(f"  concept → section edges: {in_section_count}")

# =============================================================================
# REFERENCE EDGES (existing logic)
# =============================================================================
...
for _, ref in references_df.iterrows():
    if ref["ref_type"] == "box_reference" and ref["target_exists"]:
        edges.append({
            ...
            "source_element_id": ref.get("source_element_id"),  # Provenance: element that contains the reference
...
if not anchors_df.empty and "group_id" in anchors_df.columns:
    ...
    for group_id in grouped["group_id"].dropna().unique():
        ...
        # Get source_element_id from the first member of the group
        group_source_elem = grouped[grouped["group_id"] == group_id]["source_element_id"].iloc[0] if "source_element_id" in grouped.columns else None
        for i, a1 in enumerate(group_members):
            for a2 in group_members[i+1:]:
                edges.append({
                    ...
                    "source_element_id": group_source_elem,  # Element containing the grouped header
                    "created_by": "structural",
                })
                edge_id += 1
...
typed_edge_counts = {"excludes": 0, "applies_if": 0, "defines": 0, "qualifies": 0, "requires": 0, "includes": 0}
for _, section in sections_df.iterrows():
    anchor_id = section["anchor_id"]
    # Use full_text (not body_text) to capture patterns in label + body
    full_text = section.get("full_text", "") or ""

    # Get source_box_key for requires/includes edges (box → box dependency)
    source_box_key = None
    ...
    typed_edges = extract_typed_edges_from_section(
        anchor_id=anchor_id,
        body_text=full_text,  # Pass full_text for pattern matching
    ...
    for te in typed_edges:
        ...
        edges.append({
            ...
            "source_element_id": None,  # TODO: map to specific element containing the matched sentence
            "created_by": "regex",
